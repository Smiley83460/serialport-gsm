...
module.exports = function(SerialPort) {
    ...
    if (modem.queue[0] && modem.queue[0].isBufferCommand && modem.queue[0].isWaitingBuffer) {
      try {
        ...
        if (okIndex !== -1) {
          ...
          const validBuffer = buffer.subarray(0, okIndex);
          if (validBuffer.length > 0) {
            if (!item.bufferData) {
              item.bufferData = validBuffer;
            } else {
              item.bufferData = Buffer.concat([item.bufferData, validBuffer], 
                item.bufferData.length + validBuffer.length);
            }
          }
          ...
          resultData = {
            ...
            data: item.bufferData
          };
          returnResult = true;
          item.isWaitingBuffer = false;
          item.bufferData = null; // Giải phóng bộ nhớ
          return;
        } else {
          // Nếu không tìm thấy OK, giữ lại toàn bộ buffer
          if (!item.bufferData) {
            item.bufferData = buffer;
          } else {
            item.bufferData = Buffer.concat([item.bufferData, buffer],
              item.bufferData.length + buffer.length);
          }
        }
        return;
      } catch (error) {
  ...
  modem.executeBufferCommand = function(command, callback, priority, timeout) {
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
