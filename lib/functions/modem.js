...
module.exports = function(SerialPort) {
  ...
  // Thêm biến theo dõi trạng thái SIM
  modem.simStatus = 'UNKNOWN'; // UNKNOWN, READY, NOT_READY
  ...
  modem.open = function(device, options, callback) {
    ...
    if (options.processDTMF !== undefined) {
      modem.processDTMF = options.processDTMF
      delete options.processDTMF
    }
    options.path = device
  ...
  modem.dataReceived = buffer => {
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        const buf = Buffer.from(newpart, 'binary');

        const isPrintable = buf.every(byte =>
          (byte >= 0x20 && byte <= 0x7E) || byte === 0x09 || byte === 0x0A || byte === 0x0D
        );
      
        if (isPrintable) {
          modem.logger.debug(`Modem Received: ${buf.toString('utf8')}`);
        } else {
          modem.logger.debug(`Modem Received (base64): ${buf.toString('base64')}`);
        }

        // Xử lý sự kiện SIM card
        if (newpart.includes('+CPIN:')) {
          const simStatus = newpart.includes('NOT READY') ? 'NOT_READY' : 'READY';
          
          // Chỉ emit sự kiện khi trạng thái thay đổi
          if (modem.simStatus !== simStatus) {
            const previousStatus = modem.simStatus;
            modem.simStatus = simStatus;
            
            if (simStatus === 'READY') {
              modem.emit('onSimCardInserted', {
                status: 'success',
                event: 'simCardInserted',
                data: {
                  currentStatus: simStatus,
                  previousStatus: previousStatus,
                  message: 'SIM card đã được cắm vào và sẵn sàng'
                }
              });
            } else if (simStatus === 'NOT_READY') {
              modem.emit('onSimCardRemoved', {
                status: 'warning',
                event: 'simCardRemoved',
                data: {
                  currentStatus: simStatus,
                  previousStatus: previousStatus,
                  message: 'SIM card đã được rút ra hoặc không sẵn sàng'
                }
              });
            }
          }
        ...
        if (modem.incomingSMSIndication && newpart.startsWith('+CMTI:')) { // New Message Indicator with SIM Card ID, After Receiving Read The Message From the SIM Card
          ...
          const splitted_newpart = newpart.split(',')
          let numberMatch = /"(.*?)"/g.exec(splitted_newpart[0]);
          let number = (numberMatch && numberMatch[1]) ? numberMatch[1] : null;
        ...
        else if (modem.processDTMF && newpart.startsWith('+RXDTMF')) { // +RXDTMF: 2
          const splitted_newpart = newpart.split(':')
          modem.emit('onDTMFReceived', {
            status: 'DTMF Received',
            data: {
              code: splitted_newpart[1]
            }
          })
        }
          ...
          let number = (matches && matches[1] && matches[1].trim() !== '') ? matches[1] : 'unknown';
          if (match) {
  ...
  modem.hangupCall = (callback, timeout = 1000) => {
    ...
    const item = modem.executeCommand(`AT+CHUP`, (result, error) => {
      callback(result, error)
    }, false, timeout)
    item.logic = (newpart) => {
  ...
  modem.answerCall = (callback, timeout = 1000) => {
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        modem.answerCall((result, error) => {
          if (error) {
            reject(error)
          } else {
            resolve(result)
    ...
    const item = modem.executeCommand(`ATA`, (result, error) => {
      callback(result, error)
    }, false, timeout)
    item.logic = (newpart) => {
      // console.log(`logic for command : ${item.command} with newpart : ${newpart}`)
      if((newpart === 'BEGIN' || newpart === 'OK')) {
        return {
          resultData: {
            status: 'success',
            request: 'answerCall',
        ...
        return {
          resultData: {
            status: 'fail',
            request: 'answerCall',
            data: `Cannot answer call ${newpart}`
          },
          returnResult: true
        }
      }
  ...
  modem.makeCall = (number, callback, timeout = 1000) => {
    if(typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        modem.makeCall(number, (result, error) => {
          if (error) {
            reject(error)
          } else {
            resolve(result)
    ...
    const item = modem.executeCommand(`ATD${number};`, (result, error) => {
      callback(result, error)
    }, false, timeout)
    item.logic = (newpart) => {
      // console.log(`logic for command : ${item.command} with newpart : ${newpart}`)
