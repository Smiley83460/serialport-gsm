...
module.exports = function(SerialPort) {
  ...
  modem.isConnectMode = false;
  modem.binaryData = Buffer.from('');
  ...
  modem.dataReceived = buffer => {
    ...
    modem.emit('onDataReceived', buffer);
    
    // Xử lý dữ liệu binary khi ở chế độ CONNECT
    if (modem.isConnectMode) {
      modem.binaryData = Buffer.concat([modem.binaryData, buffer]);
      // Kiểm tra nếu kết thúc bằng OK
      if (buffer.includes('OK')) {
        modem.emit('onBinaryDataReceived', modem.binaryData);
        modem.isConnectMode = false;
        modem.binaryData = Buffer.from('');
      }
      return;
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        ...
        // Kiểm tra nếu nhận được CONNECT
        if (newpart.trim() === 'CONNECT') {
          modem.isConnectMode = true;
          modem.binaryData = Buffer.from('');
          return;
        }
