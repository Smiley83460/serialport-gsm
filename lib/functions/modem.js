...
module.exports = function(SerialPort) {
  ...
  let data = Buffer.from('') // Thay đổi từ string sang buffer
  let resultData = {}
  let timeouts = {}
  ...
  modem.readSMSById = function(id, callback) {
    ...
    let resultData;
          item.logic = (input) => {
      // Lấy text từ input (có thể là string hoặc object có thuộc tính text)
      const newpart = typeof input === 'string' ? input : input.text;
      // Lấy buffer từ input nếu có
      const bufferData = typeof input === 'object' && input.buffer ? input.buffer : null;
      ...
      if (!resultData) {
        resultData = {
          ...
          request: 'readSMSById',
          data: [],
          originalBuffer: bufferData // Lưu buffer gốc nếu cần
        }
      }
  ...
  modem.release = () => {
    modem.data = Buffer.from('') //Empty the result buffer, sử dụng Buffer.from thay vì string rỗng
  ...
  modem.dataReceived = buffer => {
    // Chuyển đổi data sang Buffer nếu chưa phải
    if (!Buffer.isBuffer(data)) {
      data = Buffer.from(data || '');
    }
    // Nối buffer mới vào data hiện tại
    data = Buffer.concat([data, Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer)]);

    // Chuyển thành string để xử lý
    const dataString = data.toString();
    // Tách theo \r\n
    let parts = dataString.split('\r\n');
    // Lưu phần cuối vào data, chuyển lại thành buffer
    const lastPart = parts.pop();
    data = Buffer.from(lastPart);

    parts.forEach(part => {
      let newparts = part.split(/\r\n|\n|\r/)
      newparts.forEach(newpart => {
        if (typeof newpart === 'string' && newpart.length > 0) {
