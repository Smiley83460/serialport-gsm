...
module.exports = function(SerialPort) {
  ...
  modem.dataReceived = buffer => {
    if (Buffer.isBuffer(buffer)) {
      ...
      if (modem.queue.length && modem.queue[0] && modem.queue[0].inProgress && modem.queue[0].logic) {
        const logicResult = modem.queue[0].logic(buffer, true); // true = là buffer gốc
        if (logicResult) {
          ...
          if (returnResult) {
            returnResult = false;
            let callback = modem.queue[0].callback || null;
            ...
            if (callback) {
              setImmediate(callback, resultData);
            }
            resultData = null;
            modem.executeNext();
            return; // Thoát sau khi xử lý buffer
          }
        }
      }
    }
