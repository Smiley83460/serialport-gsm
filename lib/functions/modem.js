...
module.exports = function(SerialPort) {
  ...
  modem.dataReceived = buffer => {
    ...
    if (modem.queue[0] && modem.queue[0].isBufferCommand && modem.queue[0].isWaitingBuffer) {
      try {
        modem.queue[0].bufferData = Buffer.concat([modem.queue[0].bufferData, buffer]);
      } catch (error) {
        modem.queue[0].isWaitingBuffer = false;
        modem.queue[0].bufferData = Buffer.from('');
        resultData = {
          status: 'ERROR',
      ...
      // Kiểm tra kết thúc buffer bằng cách tìm chuỗi OK trong buffer
      let bufferStr = buffer.toString();
      console.log(bufferStr.toString('base64'))
      if (bufferStr.includes('OK')) {
        try {
          // Tách phần buffer trước OK
          let okIndex = bufferStr.indexOf('OK');
          let validBuffer = buffer.slice(0, okIndex);
          modem.queue[0].bufferData = Buffer.concat([modem.queue[0].bufferData, validBuffer]);
          
          let result = {
            resultData: {
              status: 'success',
              request: 'bufferCommand',
              data: modem.queue[0].bufferData
            },
            returnResult: true
          };
          ...
          resultData = {
            status: 'ERROR',
            request: 'bufferCommand',
            data: `Buffer finalization error: ${error.message}`
          };
          returnResult = true;
        }
        return;
