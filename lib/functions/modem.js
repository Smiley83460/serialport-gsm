...
module.exports = function(SerialPort) {
  ...
  modem.addListener = (listener) => {
    ...
    // Đảm bảo listener mới hỗ trợ cả buffer và string
    if (listener.processBuffer && typeof listener.processBuffer === 'function') {
      // Listener đã hỗ trợ buffer
    } else {
      // Tạo wrapper cho hàm process cũ để hỗ trợ buffer
      const originalProcess = listener.process;
      listener.process = (buffer, text) => {
        // Nếu text không được cung cấp, chuyển buffer thành text
        if (!text) {
          text = buffer.toString();
        }
        // Gọi hàm process ban đầu với text
  ...
  modem.readSMSById = function(id, callback) {
    ...
    let resultData;
          item.logic = (buffer) => {
      // Chuyển buffer thành string để kiểm tra các pattern
      const newpart = buffer.toString();
      ...
      if (!resultData) {
        resultData = {
          ...
          request: 'readSMSById',
          data: [],
          rawBuffer: buffer // Lưu buffer gốc
        }
      }
  ...
  modem.dataReceived = buffer => {
    // Đảm bảo buffer đầu vào là Buffer
    if (!Buffer.isBuffer(buffer)) {
      buffer = Buffer.from(buffer);
    }

    // Đảm bảo data hiện tại là Buffer
    ...
    // Tìm các điểm kết thúc lệnh (\r\n)
    let dataStr = data.toString();
    let crlfIndices = [];
    let lastIndex = -2;

    while ((lastIndex = dataStr.indexOf('\r\n', lastIndex + 2)) !== -1) {
      crlfIndices.push(lastIndex);
    }

    // Không có ký tự kết thúc, giữ lại toàn bộ buffer và chờ dữ liệu tiếp theo
    if (crlfIndices.length === 0) {
      return;
    }

    // Xử lý từng phần dữ liệu
    let startPos = 0;
    for (let i = 0; i < crlfIndices.length; i++) {
      // Cắt buffer thành từng phần
      let endPos = crlfIndices[i] + 2; // +2 cho \r\n
      // Lấy buffer của phần hiện tại
      let partBuffer = data.slice(startPos, endPos - 2); // Bỏ \r\n
      // Lấy string để log và kiểm tra
      let newpart = partBuffer.toString();
      ...
      let pduTest = /[0-9A-Fa-f]{15}/g
        ...
        modem.listeners.forEach(l => {
          // Kiểm tra match với string
          if (l.match(newpart)) {
            // Truyền buffer gốc vào hàm process
            l.process(partBuffer, newpart);
          }
        })
        ...
        if (modem.queue.length && modem.queue[0]) {
          ...
          if ((modem.queue[0].command.startsWith('AT+CMGS') || pduTest.test(modem.queue[0].command))) { // Sending of Message if with response ok.. Then Message was sent successfully.. If Error then Message Sending Failed
            ...
            if (modem.queue[0].logic && modem.queue[0].inProgress) {
              // Truyền cả buffer gốc (nếu cần) hoặc string tùy theo yêu cầu
              const originalBuffer = Buffer.isBuffer(buffer) ? buffer : Buffer.from(newpart);
              // Tạo object có cả buffer và string để logic function có thể sử dụng
              const inputData = {
                text: newpart,
                buffer: originalBuffer
              };
              const logicResult = modem.queue[0].logic(inputData);
              if (logicResult) {
    ...
    // Lưu phần buffer còn lại (nếu có) cho lần xử lý tiếp theo
