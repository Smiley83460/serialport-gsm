...
module.exports = function(SerialPort) {
  ...
  modem.maxBufferSize = 1024 * 1024; // 1MB mặc định
  modem.chunkSize = 1024; // 1KB mỗi chunk
  ...
  modem.dataReceived = buffer => {
    // Xử lý buffer command trước khi chuyển thành string
    if (modem.queue[0] && modem.queue[0].isBufferCommand && modem.queue[0].isWaitingBuffer) {
      try {
        ...
        if (item.totalSize + buffer.length > modem.maxBufferSize) {
          throw new Error(`Buffer size exceeded limit of ${modem.maxBufferSize} bytes`);
        }

        // Thêm chunk mới
        item.chunks.push(buffer);
        item.totalSize += buffer.length;

        // Kiểm tra kết thúc
        let bufferStr = buffer.toString();
        if (bufferStr.includes('OK')) {
          let okIndex = bufferStr.indexOf('OK');
          let validBuffer = buffer.slice(0, okIndex);
          
          if (validBuffer.length > 0) {
            item.chunks.push(validBuffer);
            item.totalSize += validBuffer.length;
          }
          ...
          let result = {
            resultData: {
              ...
              data: Buffer.concat(item.chunks)
            },
            returnResult: true
          };
        ...
        modem.queue[0].isWaitingBuffer = false;
        modem.queue[0].chunks = [];
        modem.queue[0].totalSize = 0;
        resultData = {
          status: 'ERROR',
          request: 'bufferCommand',
  ...
  modem.executeBufferCommand = function(command, callback, priority, timeout, options = {}) {
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        ...
        }, priority, timeout, options)
      })
    }

    // Cho phép tùy chỉnh kích thước buffer
    const maxBufferSize = options.maxBufferSize || modem.maxBufferSize;
    const chunkSize = options.chunkSize || modem.chunkSize;
    ...
    item.logic = (newpart) => {
      if (item.isWaitingBuffer) {
        if (newpart.includes('OK')) {
          item.isWaitingBuffer = false;
          try {
            // Ghép tất cả chunks thành buffer cuối cùng
            const finalBuffer = Buffer.concat(item.chunks);
            item.chunks = []; // Giải phóng bộ nhớ
            item.totalSize = 0;
            return {
              resultData: {
                status: 'success', 
            ...
            return {
              resultData: {
                status: 'ERROR',
                request: 'bufferCommand',
                data: `Failed to process buffer: ${error.message}`
              },
              returnResult: true
            }
          }
