...
module.exports = function(SerialPort) {
  ...
  modem.bufferMode = false;
  modem.bufferData = null;
  ...
  modem.dataReceived = buffer => {
    // Xử lý buffer command với streaming
    if (modem.queue[0] && modem.queue[0].isBufferCommand && modem.queue[0].isWaitingBuffer) {
      const item = modem.queue[0];
      
      try {
        // Tìm vị trí OK trong buffer mới
        const okIndex = buffer.indexOf(Buffer.from('OK'));
        let dataToProcess = buffer;
        
        if (okIndex !== -1) {
          // Nếu tìm thấy OK, chỉ lấy phần trước OK
          dataToProcess = buffer.subarray(0, okIndex);
        }
        
        // Stream data ngay lập tức nếu có callback
        if (item.streamCallback && dataToProcess.length > 0) {
          item.streamCallback(dataToProcess, false); // false = chưa kết thúc
          item.totalReceived += dataToProcess.length;
        }
        
        // Lưu buffer nếu cần (optional)
        if (!item.streamCallback) {
          if (!item.bufferData) {
            item.bufferData = dataToProcess;
          } else {
            item.bufferData = Buffer.concat([item.bufferData, dataToProcess],
              item.bufferData.length + dataToProcess.length);
          }
        } else {
          // Nếu có stream callback, chỉ lưu buffer nhỏ để kiểm tra OK
          item.bufferData = dataToProcess;
        }
        ...
        if (okIndex !== -1) {
          ...
          if (item.streamCallback) {
            item.streamCallback(null, true); // true = đã kết thúc
          }
          
          resultData = {
            status: 'success',
            request: 'bufferCommand',
            data: item.streamCallback ? null : item.bufferData,
            totalBytes: item.totalReceived
          };
          returnResult = true;
          item.isWaitingBuffer = false;
          item.bufferData = null;
          item.streamCallback = null;
        }
        ...
        resultData = {
          status: 'ERROR',
          request: 'bufferCommand',
          data: `Buffer processing error: ${error.message}`
        };
        returnResult = true;
        return;
      }
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        ...
        // Kiểm tra CONNECT cho buffer command
        if (modem.queue[0] && modem.queue[0].isBufferCommand && newpart.trim() === 'CONNECT') {
          modem.queue[0].isWaitingBuffer = true;
          modem.queue[0].bufferData = null;
          return;
        }
        ...
        else if (newpart.startsWith('+CLCC')) {
          const match = /\+CLCC:\s*\d+,\d+,(6),/.exec(newpart); // 6 là trạng thái 'disconnected'
          const matches = /\+CLCC:\s*\d+,\d+,\d+,\d+,\d+,"(.+?)"/.exec(newpart);
          if (match && matches) {
            modem.emit('onCallEnded', {
              status: 'Call Ended',
              data: {
                number: matches[1],
                reason: 'CLCC'
  ...
  modem.executeBufferCommand = function(command, callback, priority, timeout, streamCallback) {
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        modem.executeBufferCommand(command, (result, error) => {
          if (error) {
            reject(error)
          } else {
            resolve(result)
          }
        }, priority, timeout, streamCallback)
      })
    }
    
    const item = modem.executeCommand(command, (result, error) => {
      // Đảm bảo xóa trạng thái buffer khi callback được gọi
      if (item.isWaitingBuffer) {
        item.isWaitingBuffer = false;
        item.bufferData = null;
        item.streamCallback = null;
      }
    ...
    item.isBufferCommand = true;
    item.bufferData = null;
    item.streamCallback = streamCallback; // Callback để stream data
    item.totalReceived = 0;
    
    // Thêm handler riêng cho timeout của buffer command
    item.on('timeout', () => {
      if (item.isWaitingBuffer) {
        item.isWaitingBuffer = false;
        item.bufferData = null;
      ...
      callback(undefined, new Error('Buffer command timeout'));
    });
    
    item.logic = (newpart) => {
      if (item.isWaitingBuffer) {
        // Nếu đang trong chế độ nhận buffer
        if (newpart.includes('OK')) {
          item.isWaitingBuffer = false;
          const finalData = item.bufferData;
          item.bufferData = null;
          item.streamCallback = null;
          return {
            resultData: {
      ...
      // Bắt đầu nhận buffer khi thấy CONNECT
      if (newpart.trim() === 'CONNECT') {
        item.isWaitingBuffer = true;
        item.bufferData = null;
        item.totalReceived = 0;
        return;
      }
